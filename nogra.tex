...
\textbf{(a)} If $\mathcal{A}$ is any logical statement, then
$\ive{\text{not } \mathcal{A}} = 1 - \ive{\mathcal{A}}$.

\textbf{(b)} If $\mathcal{A}$ and $\mathcal{B}$ are two logical
statements, then
...
\]
(See Theorem~\ref{thm.dominating.heinrich} for the definition
of a ``detached pair''.)
\end{lemma}
