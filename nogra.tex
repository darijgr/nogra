...
%EndExpansion
at \DTMcurrenttime\ 
(unfinished draft!)\\\vspace{1pc}
These notes are \textbf{frozen} in a (very) unfinished state.
Currently, only two chapters (beyond the preface) exist, and
they too are incomplete (although hopefully readable).
The chapter on dominating sets is perhaps the only real
finished content here.
See \cite{22s} for a complete set of notes on graph theory
(though with a somewhat different focus).
}
\maketitle
\tableofcontents
...
$u - v \geq 3$ is satisfied, whereas if we write $\set{1, 5}$ as
...
unambiguously reconstruct the graph $G$ from the picture.
(Thus, for example:
\begin{itemize}
...
unambiguously reconstruct the graph $G$ from the picture'' criterion):
\begin{align*}
\xymatrix{
...
$R\tup{r,s}$; thus, we have just shown that $R\tup{3,3} = 6$.
...
\begin{align*}
G_1 &= \tup{ \set{1, 2}, \set{12} }, \\
G_2 &= \tup{ \set{1, 2}, \set{12} }, \\
G_3 &= \tup{ \set{1, 2, 3, 4}, \set{12, 23, 34, 13, 14} }.
\end{align*}
...
\begin{example}
Recall the graph $G$ from Example~\ref{exa.intro.discon-graph},
...
$a_0 = u$). This proves Proposition~\ref{prop.walks.rev} \textbf{(a)}.
...
as we did in Example~\ref{exa.walk.decrease}:
...
of this walk $\mathbf{a}$. Then, there exists a walk from $u$ to $v$ of
...
Definition~\ref{def.intro.cycle-graph} has $2n$ cycles
(for $n \geq 3$):
...
What is the smallest possible size of a dominating
...
$V$ of (finitely many) people each of whom has at least one friend
(among the others),
...
However, if $\tup{A, B}$ is a detached pair, then the condition
...
The following property of independent sets is almost trivial:
...
\begin{itemize}
 \item There are, most likely, no quick algorithms to find
       ...
       known (and none is possible, unless $P = NP$).
...
\newline
See also \href{https://arxiv.org/abs/2008.09862v3}{arXiv:2008.09862v3}
for an older version.

\bibitem[Grinbe25]{22s}Darij Grinberg, \textit{An introduction to graph theory
(Text for Math 530 in Spring 2022 at Drexel University)}, 12 April 2026.
\newline\url{http://www.cip.ifi.lmu.de/~grinberg/t/22s/graphs.pdf} .
\newline
See also \href{https://arxiv.org/abs/2308.04512v3}{arXiv:2308.04512v3}
for an older version.
...
\newline\url{https://richardhammack.github.io/BookOfProof/}
...
\bibitem[LeLeMe18]{LeLeMe16}
Eric Lehman, F. Thomson Leighton, Albert R. Meyer,
\textit{Mathematics for Computer Science},
revised Wednesday 6th June 2018,
...
\newline \url{https://neilstrickland.github.io/linear_maths/}
