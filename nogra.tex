...
we say the graphs $G$ and $H$ are said to be \textit{isomorphic}, and
we write $G \cong H$. Sometimes, the relation $G \cong H$ itself will
be called an ``isomorphism'' (although it is not a map).
\end{definition}

Given this definition, we can now rigorously state our relation
between the two graphs $\tup{\set{1,2,3},\set{\set{1,2}}}$ and
$\tup{\set{1,2,3},\set{\set{1,3}}}$ observed above. Namely, if we
...
obtain a map $\phi' : \edges{G} \to \edges{H}$. This map $\phi'$ is
...
$\set{\phi{x} \ \mid \ x \in e}$ of $S$). Now, let $H$ be the simple
graph $\tup{S, F}$. Then, $\verts{H} = S$. Hence, $\phi$ is a
...
\begin{todo}
Derive Theorem~\ref{thm.dominating.brouwer} from
...
another example,
$\ive{\text{Proposition~\ref{prop.dominating.|V|/2} holds}}
...
$P = \varnothing$ is false), we obtain
$\sum_{\substack{A \subseteq P}} \tup{-1}^{\abs{A}}
...
\subsection{\label{subsect.dominating.lemmas}Lemmas for the proof of
the Heinrich-Tittmann formula}

In this section, we shall prove some further results that will be used
in our proof of Theorem~\ref{thm.dominating.heinrich}. Throughout this
