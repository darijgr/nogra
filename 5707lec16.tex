...
%TCIDATA{LastRevised=Monday, December 04, 2017 09:56:47}
...
e.g., \cite[\S 12.5.2]{LeLeMe17}). We shall derive it from Theorem \ref{thm.3b}.
...
We can now prove Theorem \ref{thm.hall}:
...
Consider the network $N$. It clearly has the property that $c\left(  a\right)
\in\mathbb{N}$ for each $a\in A$ (since $c\left(  a\right)  =1$ for each $a\in
A$). Hence, Lemma \ref{lem.3bat} shows that there exist an integer flow
$f:A\rightarrow\mathbb{Q}_{+}$ and a subset $S$ of $V$ satisfying $s\in S$ and
$t\notin S$ and $c\left(  S,\overline{S}\right)  =\left\vert f\right\vert $.
Consider these $f$ and $S$.
...
S\right\vert \geq\left\vert N\left(  U\right)  \right\vert \geq\left\vert
U\right\vert \label{pf.thm.hall.hard.3}%
\end{equation}
(by (\ref{pf.thm.hall.hard.ass})).
...
\setminus S\right\vert }_{\substack{\geq\left\vert U\right\vert \\\text{(by
(\ref{pf.thm.hall.hard.3}))}}}+\underbrace{\left\vert X\setminus S\right\vert
}_{\substack{=\left\vert X\right\vert -\left\vert U\right\vert \\\text{(by
(\ref{pf.thm.hall.hard.4}))}}}\\
&  \geq\left\vert U\right\vert +\left\vert X\right\vert -\left\vert
U\right\vert =\left\vert X\right\vert .
\end{align*}
Hence, (\ref{pf.thm.hall.hard.5}) yields $\left\vert f\right\vert =\left\vert
\left[  S,\overline{S}\right]  \right\vert \geq\left\vert X\right\vert $.
...
Proposition \ref{prop.hall.lem1} \textbf{(e)} shows that $\left\vert
M\right\vert =\left\vert f\right\vert \geq\left\vert X\right\vert $. Hence,
Lemma \ref{lem.hall.complete-mat} \textbf{(b)} yields that the matching $M$ is
$X$-complete. Hence, $G$ has an $X$-complete matching (namely, $M$). This
proves the $\Longleftarrow$ direction of Theorem \ref{thm.hall}. Thus, the
proof of Theorem \ref{thm.hall} is complete.
\end{proof}
