...
%TCIDATA{LastRevised=Friday, April 24, 2026 16:36:40}
...
\cite[Chapter 4]{Schrij17}. Also, \cite[\S 11.2]{Martin17} gives a neat
...
We shall mostly follow \cite[\S 11.2]{Martin17}. In particular, we will only
...
Proposition \ref{prop.2} \textbf{(d)}.
\end{proof}
...
\end{align*}
In particular, the left-hand side of this equation is well-defined (i.e.,
there exists an integer flow $f$ for which $\left\vert f\right\vert $ is maximum).
\end{theorem}
...
exists a subset $S$ of $V$ satisfying $s\in S$ and $t\notin S$ and $c\left(
...
belong to $A^{-1}$ must be colored red; every arc $b\in P$ that does not
belong to $A$ must be colored blue; every arc $b\in P$ that belongs to both $A$
...
and }d\text{ is red}\right)  $ or $\left(  d^{-1}\in P\text{ and }d^{-1}
\text{ is blue}\right)  $.
\par
...
\par
If $\left(  d^{-1}\in P\text{ and }d^{-1}\text{ is blue}\right)  $, then $d^{-1}$
is either $x$ or $y$ (since $d^{-1}\in P$, but the only arcs in $P$ having source
...
must prove that there exists a subset $S$ of $V$ satisfying $s\in S$ and
...
It thus remains to show that $c\left(  S,\overline{S}\right)  =
...
source and the sink of $N$ shall be $s$ and $t$, respectively. The capacity
...
\bibitem[Martin18]{Martin17}Jeremy L. Martin, \textit{Lecture Notes on
Algebraic Combinatorics}, 28 November 2018.\newline\url{https://web.archive.org/web/20190110212210/http://jlmartin.faculty.ku.edu/~jlmartin/LectureNotes.pdf}
