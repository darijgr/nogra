...
%TCIDATA{LastRevised=Friday, April 24, 2026 16:33:31}
...
\]
(by (\ref{pf.thm.hamp.Dbar.5})). Thus,%
\[
\left(  \#\text{ of hamps of }\overline{D}\right)  \equiv\left(  \#\text{ of
...
,\ldots,x_{a},v,y_{1},y_{2},\ldots,y_{b}\right)  $ is a hamp of $D$. Thus, $D$
...
\cite[solution to problem 7.8]{Tomesc85}).\footnote{Other proofs appear in
\cite[Corollaire 5.1]{Lass02}, \cite{GriSta23} and \cite{IrvOma25}.} We have
already done most of the hard work when we proved Theorem \ref{thm.hamp.Dbar},
which will come in useful in the proof; but we will need one more lemma:
...
use the arc $vw$). Thus, any hamp of $D_{2}$ that uses the arc $wv$ is a hamp
of $D^{\prime}$ that uses the arc $wv$. Conversely, as we have already shown,
any hamp of $D^{\prime}$ that uses the arc $wv$ is a hamp of $D_{2}$ that uses
the arc $wv$. Combining the results of the previous two sentences, we see that
the hamps of $D_{2}$ that use the arc $wv$ are precisely the hamps of
$D^{\prime}$ that use the arc $wv$. Hence,%
\begin{align}
...
numbers is still open. See \cite{MO232751} for more about this peculiar question.

For refinements and generalization of Theorems \ref{thm.hamp.Dbar} and Theorem
\ref{thm.tourn.redei}, see \cite{GriSta23} and \cite{IrvOma25}.
...
\bibitem[GriSta23]{GriSta23}\href{https://arxiv.org/abs/2307.05569}{Darij
Grinberg, Richard P. Stanley, \textit{The Redei--Berge symmetric function of a
directed graph} arXiv:2307.05569}.

\bibitem[IrvOma25]{IrvOma25}\href{https://doi.org/10.37236/13841}{John Irving,
Mohamed Omar, \textit{Revisiting the R\'{e}dei-Berge Symmetric Functions via
Matrix Algebra}, The Electronic Journal of Combinatorics \textbf{32} (2025),
issue 4, \#P4.43.}
